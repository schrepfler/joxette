...
- [~] **Build the operator** — JOSDK module (Phases 1–3 in `operator-design.md`)
  ...
  - [x] Phase 3 — (1) clusterRef-not-ready rescheduling: API reconcilers probe JoxetteRestClient.ready() and reschedule Progressing if the target cluster isn't serving; (2) deploy manifests deploy/operator/ (namespace, SA, cluster RBAC, Deployment, Service) + micrometer-registry-prometheus + ServiceMonitor; (3) validating admission webhook (/validate/joxettecluster runs CatalogGuardrail at apply-time) + cert-manager webhook manifests; (4) OpenAPI contract guard instead of a generated client — SpringDocIT exports docs/openapi.json (73 paths), OpenApiContractTest asserts the operator's endpoints exist in it. Operator suite 46 tests (commits through Phase 3). HPA not added (replay-tier HPA belongs to the workload chart, not the operator pod).
