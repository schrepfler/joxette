...
@ActiveProfiles("it")
@Testcontainers
class EntityStorageStatsFlushedSizeIT {

    private static final String BUCKET = "joxette-entity-stats-flushed-test";
    ...
    @Test
    void getEntityTypeStorageStats_reportsTotalFlushedSizeBytes() throws Exception {
        try (Statement st = duckDB.createStatement()) {
            st.execute("CREATE TABLE IF NOT EXISTS lake.main.entity_order (" +
                    ...
                    "kafka_key VARCHAR, kafka_value BLOB, metadata VARCHAR, headers STRUCT(key VARCHAR, value VARCHAR)[])");
            st.execute("INSERT INTO lake.main.entity_order (recorded_at, entity_id, bucket, topic, " +
                    "kafka_offset, kafka_partition, kafka_timestamp, kafka_key, kafka_value, headers) " +
                    "VALUES (now(), 'ORD-1', 0, 'orders.events', 0, 0, now(), 'k', 'v', [])");
            st.execute("CALL ducklake_flush_inlined_data('lake')");
            st.execute("CHECKPOINT");
        }

        var stats = lifecycleService.getEntityTypeStorageStats("order");

        assertThat(stats.totalRows()).isEqualTo(1L);
        assertThat(stats.totalFlushedSizeBytes()).isGreaterThan(0);
    }
}
