...
| Whole catalog lost | Parquet data on object storage survives. Rebuild config via snapshot restore. Rebuild `known_entities` via `POST /cassettes/entities/rebuild-known-entities?recoverOrphanedFiles=true` — **the `recoverOrphanedFiles=true` flag is required for this scenario.** Without it the endpoint treats every entity table's zero row count as an intentional deletion (GDPR erase, truncate, retention) rather than catalog loss and rebuilds nothing, because "catalog lost, table recreated empty" and "this entity type genuinely never had data" are structurally indistinguishable from the zero-row count alone — the flag is how an operator asserts, after confirming catalog loss, which one this is. See [`catalog-scaling.md`](catalog-scaling.md). |
